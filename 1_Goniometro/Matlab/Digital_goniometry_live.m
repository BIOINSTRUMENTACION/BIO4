...
time = 1;             
while 1                       % Iterate over Frames
    %%%% PRERPOCESS IMAGE %%%
    temp =  snapshot(cam);                  % Get frame
    ...
    [r,c]=size(centroids);                                  % Get shape of matrix of centroid 
    ...
    time = time + 1;                % Get current Frame time
    A = [A angle];                  % Append angle to vector of angles
    T = [T time];                   % Append time to vector of time
